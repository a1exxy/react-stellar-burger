...
export default function OrderDescription() {
  ...
  useEffect(()=>{
    getOrder({dispatch:dispatch, orderId: id})
  },[id])
  ...
  const ingredients = useMemo(() => {
      ...
      for (let key in ingredientsHash) {
        const item = storeIngredients.find(e => e._id === key)
  ...
  return (
    <div className={styles.page}>
      <div className={styles.orderDetails}>
        ...
        {order && order.status === 'done' ?  <p className={`${styles.doneOrderNumber} text text_type_main-default`}>Выполнен</p> : ''}
        <h2 className={'text text_type_main-medium mt-15 mb-6'}>Состав:</h2>
        <ul className={`custom-scroll ${styles.elementsBlock}`}>
            {ingredients.elements.map((item, index) => {
